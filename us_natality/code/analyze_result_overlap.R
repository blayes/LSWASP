...
#rm(list=ls())
setwd("/Users/AaronXu/Research_Project/Mixed effect Model/Meeting12 -- new comb/real_accur_new")
# each res stores one file: {dls, newdls,XL_dls}*{dep250, dep500}*{20,30,50,100}
res <- vector("list",24)
...
partition <- c("dep250","dep500")
l <- list(m = 1:3, s = 1:4, p = 1:2)
comb <- expand.grid(l)
for(i in 1:24){
...
comb$p <- ifelse(comb$p==1, "250", "500")
...
## also need to run this part in real_making_the_plot_disjoint.R to get accur_Rmat_2D_disjoint, accur_Rmat_3D_disjoint
## to make the R matrix plot 
...
accur_list <- list(accur_beta_2D,accur_beta_3D,accur_beta_4D,accur_beta_5D,accur_beta_6D,
                   accur_dmat_2D,accur_dmat_3D,accur_dmat_4D,accur_dmat_5D,accur_dmat_6D,
                   accur_Rmat_2D,accur_Rmat_3D)
summary_list_overlap <- vector("list",length(accur_list))
names(summary_list_overlap) <- c("sum_beta_2D","sum_beta_3D","sum_beta_4D","sum_beta_5D","sum_beta_6D",
                         "sum_dmat_2D","sum_dmat_3D","sum_dmat_4D","sum_dmat_5D","sum_dmat_6D",
                         "sum_Rmat_2D","sum_Rmat_3D")
...
for(accur_temp in accur_list){
  ...
  summary_temp <- summary_temp[,c("m","p","mean.20","sd.20","mean.30","sd.30","mean.50","sd.50","mean.100","sd.100")]
  summary_list_overlap[[sum_ind]] <- summary_temp
  sum_ind <- sum_ind + 1 
}
...
sum_list_overlap <- names(summary_list_overlap)
# beta - dep 250
sum_name_list <- sum_list_overlap[1:5]
sum_dat_list <- summary_list_overlap
sum_ind_overlap <- "250"
partition <- c(20,30,50,100)
...
# D - dep
sum_name_list <- sum_list_overlap[6:10]
sum_dat_list <- summary_list_overlap
sum_ind_overlap <- "250"
partition <- c(20,30,50,100)
...
# R - dep
sum_name_list <- sum_list_overlap[11:12]
sum_dat_list <- summary_list_overlap
sum_ind_overlap <- "250"
partition <- c(20,30,50,100)
