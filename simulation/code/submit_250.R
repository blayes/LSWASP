...
if (mtd == 1) {
    ...
    fname <- paste0("../data/part_data_rep_", cid, "_k_20_dep_samp250.rds")
    dat <- readRDS(fname)
    train <- dat[[sid]]
    ...
    fname <- paste0("~/simulation/result/samp_250/sub20/dep_sim_dls_rep_", cid, "_nsub_", sid, "_k_20_samp_250.rds")
    saveRDS(res, fname)
} else if (mtd == 2) {
    source("dls_sampler.R")
    ...
    fname <- paste0("../data/part_data_rep_", cid, "_k_50_dep_samp250.rds")
    dat <- readRDS(fname)
    train <- dat[[sid]]
    ...
    fname <- paste0("~/simulation/result/samp_250/sub50/dep_sim_dls_rep_", cid, "_nsub_", sid, "_k_50_samp_250.rds")
    saveRDS(res, fname)
} else if (mtd == 3) {
    source("dls_sampler.R")
    ...
    fname <- paste0("../data/part_data_rep_", cid, "_k_100_dep_samp250.rds")
    dat <- readRDS(fname)
    train <- dat[[sid]]
    ...
    fname <- paste0("~/simulation/result/samp_250/sub100/dep_sim_dls_rep_", cid, "_nsub_", sid, "_k_100_samp_250.rds")
    saveRDS(res, fname)
} else if (mtd == 4) {
    source("dls_sampler.R")
    ...
    fname <- paste0("../data/part_data_rep_", cid, "_k_150_dep_samp250.rds")
    dat <- readRDS(fname)
    train <- dat[[sid]]
    ...
    fname <- paste0("~/simulation/result/samp_250/sub150/dep_sim_dls_rep_", cid, "_nsub_", sid, "_k_150_samp_250.rds")
    saveRDS(res, fname)
} else if (mtd == 5) {
    source("dls_sampler.R")
    ...
    fname <- paste0("../data/part_data_rep_", cid, "_k_200_dep_samp250.rds")
    dat <- readRDS(fname)
    train <- dat[[sid]]
    ...
    fname <- paste0("~/simulation/result/samp_250/sub200/dep_sim_dls_rep_", cid, "_nsub_", sid, "_k_200_samp_250.rds")
    saveRDS(res, fname)
}else if (mtd == 6) {
    source("dls_comb.R")
    ...
    cvs <- 1:10
    cid <- cvs[id]
    # dep_sim_dls_rep_7_nsub_3_k_100_samp_250.rds
    for (nsub in c(20, 50, 100, 150, 200)) {
        res <- list()
        for (sid in 1:nsub) {
            foldname <- paste0("sub",nsub)
            fname <- paste0("~/simulation/result/samp_250/",foldname,"/dep_sim_dls_rep_", cid, "_nsub_", sid, "_k_", nsub, "_samp_250.rds")
        ...
        rname <- paste0("~/simulation/result/dep_dls_samps_rep_", cid, "_k_", nsub, "_samp_250.rds")
 ...
 #########  to get accuracy measure for samp 500 #############
 ...
 ######### DLS
  ...
  for(ii in nrep){
    data_name <- paste0("../result/comb/dep_dls_samps_rep_",ii,"_k_", subset, "_samp_250.rds")
  ...
  for(jj in 1:10){
    ...
    for(jjj in 2:4){
      ...
      for(jjjj in ncol(nD_beta)){
        accur_beta_temp_nD <- Accuracy(res[[jj]]$betasNew$wasp[,nD_beta[,jjjj]],resfull[[jj]]$beta[,nD_beta[,jjjj]], nobs) +accur_beta_temp_nD
      }
      if(jjj ==2){
  ...
  fname <- paste0("~/simulation/result/sim_dls_k_", subset, "_accuracy_dep250.rds")
  ...
  fname <- paste0("~/simulation/result/sim_XL_dls_k_", subset, "_accuracy_dep250.rds")
  saveRDS(result, fname)
